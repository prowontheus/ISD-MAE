...
if __name__ == '__main__':
    ...
    ##################################################Task06_Lung######################################################
    # ori_path = '/home/jerry/datasets/Task06_Lung/imagesTr'
    # dst_path = '/home/jerry/datasets/Task06_Lung/Task06_Lung2D/train'
    # image_list = glob.glob(f'{ori_path}/**/*.nii.gz', recursive=True)
    # label_list = [file.replace('imagesTr', 'labelsTr') for file in image_list]
    # open_fail = 0
    # size_fail = 0
    # space_fail = 0
    # resample_fail = 0
    ...
    #     try:
    #         itk_img1 = sitk.ReadImage(image_list[i])
    #         itk_img2 = sitk.ReadImage(label_list[i])
    #     except:
    #         open_fail += 1
    #         print(f'open_fail={open_fail}')
    #         continue
    #
    #     spacing_1 = itk_img1.GetSpacing()
    #     spacing_2 = itk_img2.GetSpacing()
    #     size_1 = itk_img1.GetSize()
    #     size_2 = itk_img2.GetSize()
    #
    ...
    #         continue
    #     if spacing_1 != spacing_2:
    #         print('1')
    #         itk_img2.SetSpacing(spacing_1)
    #         spacing_2 = itk_img2.GetSpacing()
    #         if spacing_1 != spacing_2:
    #             space_fail += 1
    ...
    #     resampled_sitk_img = resample_img(itk_img1, out_spacing=new_spacing, is_label=False)
    #     resampled_sitk_lbl = resample_img(itk_img2, out_spacing=new_spacing, is_label=True)
    #     if resampled_sitk_img.GetSpacing() != (1.0, 1.0, 1.0) or resampled_sitk_lbl.GetSpacing() != (1.0, 1.0, 1.0) or resampled_sitk_img.GetSize() != resampled_sitk_lbl.GetSize():
    #         resample_fail += 1
    ...
    #     filename = os.path.basename(image_list[i])[:-len('.nii.gz')]
    #     crop3DTo2D(sitk_array, sitk_mask, 'crop', dst_path, filename, 10)
    # print(size_list)
    # print(f'open_fail={open_fail}')
    # print(f'size_fail={size_fail}')
    # print(f'space_fail={space_fail}')
    ...
    ##################################################COVID19_CT######################################################
    # ori_path = '/home/jerry/datasets/COVID-19 CT scans/ct_scans'
    # dst_path = '/home/jerry/datasets/COVID-19 CT scans/2D/train'
    # image_list = glob.glob(f'{ori_path}/**/*.nii', recursive=True)
    # label_list = [file.replace('ct_scans', 'infection_mask') for file in image_list]
    # open_fail = 0
    # size_fail = 0
    # space_fail = 0
    # resample_fail = 0
    ...
    #     sitk_array = np.moveaxis(sitk_array, 0, 2)
    #     sitk_mask = np.moveaxis(sitk_mask, 0, 2)
    #     if np.max(sitk_mask) > 1:
    #         sitk_array = sitk_array[:,:,:-3]
    #         sitk_mask = sitk_mask[:,:,:-3]
    #     filename = os.path.basename(image_list[i])[:-len('.nii')]
    #     crop3DTo2D(sitk_array, sitk_mask, 'crop', dst_path, filename, 10)
    # print(size_list)
    # print(f'open_fail={open_fail}')
    # print(f'size_fail={size_fail}')
    ...
    ##################################################COVID19_3D######################################################
    # ori_path = '/home/jerry/datasets/image_text_clinical/covid19'
    # dst_path = '/home/jerry/datasets/image_text_clinical/covid19/2D'
    # label_list = glob.glob(f'{ori_path}/**/*_mask.nii.gz', recursive=True)
    # image_list = [file.replace('_mask.nii.gz', '.nii.gz') for file in label_list]
    # open_fail = 0
    # size_fail = 0
    # space_fail = 0
    # resample_fail = 0
    ...
    ###############################################TotalSegmentator####################################################
    ori_path = '/home/jerry/datasets/Totalsegmentator_dataset_v201'
    dst_path = '/home/jerry/datasets/TotalSegmentator2D/train'
