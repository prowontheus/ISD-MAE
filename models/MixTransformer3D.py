...
from segmentation_models_pytorch.encoders.mix_transformer import EncoderMixin
...
class OverlapPatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        i_size = (img_size, img_size, img_size)
        p_size = (patch_size, patch_size, patch_size)

        self.img_size = img_size
        self.patch_size = patch_size
        self.H, self.W, self.D = i_size[0] // p_size[0], i_size[1] // p_size[1], i_size[2] // p_size[2]
        ...
        self.proj = nn.Conv3d(
            ...
            stride=stride,
            padding=(p_size[0] // 2, p_size[1] // 2, p_size[2] // 2),
        )
...
class MixVisionTransformerEncoder3D(MixVisionTransformer3D, EncoderMixin):
...
mix_transformer_encoders = {
    "mit_b0": {
        "encoder": MixVisionTransformerEncoder3D,
        "pretrained_settings": {
...
def generate_mit_encoder(name, in_channels=3, depth=5, weights=None, output_stride=32, **kwargs):
    Encoder = mix_transformer_encoders[name]['encoder']
